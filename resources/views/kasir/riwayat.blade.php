        ...
        <div x-data="{ 
            ...
            <div x-show="showExportMenu" 
                ...
                <div class="p-3 border-b border-gray-100">
                    ...
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=excel&period=1day&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">
                        📅 Hari Ini
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=excel&period=1week&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">
                        📊 1 Minggu Terakhir
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=excel&period=1month&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">
                        📈 1 Bulan Terakhir
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=excel&period=lifetime&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">
                ...
                <div class="p-3">
                    ...
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=pdf&period=1day&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">
                        📅 Hari Ini
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=pdf&period=1week&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">
                        📊 1 Minggu Terakhir
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=pdf&period=1month&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">
                        📈 1 Bulan Terakhir
                    </a>
                    <a :href="`{{ route('kasir.riwayat.export') }}?format=pdf&period=lifetime&customer_id=${customerId}&product_id=${productId}`" 
                       class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">
                        🗂️ Semua Transaksi (Lifetime)
                    </a>
                </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="date" name="date_from" value="{{ request('date_from') }}" max="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded px-3 py-2">
            </div>
            <div>
                <label class="block text-sm font-medium mb-2">Sampai Tanggal</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" max="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded px-3 py-2">
            </div>
            <div class="flex items-end">
                <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700 mr-2">Filter</button>
                <a href="{{ route('kasir.riwayat') }}" class="border border-gray-300 px-6 py-2 rounded hover:bg-gray-50">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm">
                        @if($transaction->member)
                            <span class="text-gray-900">{{ $transaction->member->email }}</span>
                        @else
                            <span class="text-gray-400 italic">-</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-sm font-semibold">Rp {{ number_format($transaction->total, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 text-sm capitalize">{{ $transaction->payment_method ?? 'cash' }}</td>
                    <td class="px-6 py-4 text-sm">
                        @if($transaction->member_id && $transaction->is_verified)
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('kasir.cetak-struk', $transaction->id) }}" 
                           target="_blank"
