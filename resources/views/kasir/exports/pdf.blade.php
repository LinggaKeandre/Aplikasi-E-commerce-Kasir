...
<body>
    ...
    <div class="info-section">
        <div class="info-row">
            ...
            <div class="info-value">{{ $kasir->name }}</div>
        </div>
        <div class="info-row">
            ...
            @forelse($transactions as $transaction)
                ...
                @foreach($transaction->items as $index => $item)
                    <tr>
                        ...
                        @if($firstRow)
                            <td class="text-center" rowspan="{{ $itemCount }}">{{ ucfirst($transaction->payment_method ?? 'cash') }}</td>
                            <td class="text-center" rowspan="{{ $itemCount }}">
                                @if($transaction->is_verified)
                                    <span class="badge badge-success">✓ {{ $transaction->points_awarded }} poin</span>
                                @else
                ...
                @if($itemCount === 0)
                    <tr>
                        ...
                        <td class="text-center">{{ ucfirst($transaction->payment_method ?? 'cash') }}</td>
                ...
                <tr class="subtotal-row">
                    <td colspan="8" class="text-right" style="padding-right: 10px;">TOTAL ORDER #{{ $transaction->order_number }}</td>
    ...
    <div class="footer">
        ...
        <p>Dicetak pada: {{ $printDate }} oleh {{ $kasir->name }}</p>
    </div>
</body>
</html>
