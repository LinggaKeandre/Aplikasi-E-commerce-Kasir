...
<div class="max-w-7xl mx-auto py-6">
            ...
            <div x-show="showExportMenu" 
                ...
                <div x-show="selectedType" class="p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="space-y-1">
                            <a :href="`{{ route('admin.reports.export') }}?format=excel&type=${selectedType}&period=1day&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">📅 Hari Ini</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=excel&type=${selectedType}&period=1week&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">📊 1 Minggu</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=excel&type=${selectedType}&period=1month&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-green-50 rounded transition">📈 1 Bulan</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=excel&type=${selectedType}&period=lifetime&customer_id=${customerId}&product_id=${productId}`" 
                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            <a :href="`{{ route('admin.reports.export') }}?format=pdf&type=${selectedType}&period=1day&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">📅 Hari Ini</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=pdf&type=${selectedType}&period=1week&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">📊 1 Minggu</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=pdf&type=${selectedType}&period=1month&customer_id=${customerId}&product_id=${productId}`" 
                               class="block px-3 py-2 text-sm text-gray-700 hover:bg-red-50 rounded transition">📈 1 Bulan</a>
                            <a :href="`{{ route('admin.reports.export') }}?format=pdf&type=${selectedType}&period=lifetime&customer_id=${customerId}&product_id=${productId}`" 
        ...
        <div class="bg-white rounded-lg shadow p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 mt-1">{{ $stats['kasir_transactions'] }} transaksi</p>
                    <p class="text-xs text-gray-500">Rp {{ number_format($stats['kasir_revenue'], 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 mt-1">{{ $stats['online_transactions'] }} transaksi</p>
                    <p class="text-xs text-gray-500">Rp {{ number_format($stats['online_revenue'], 0, ',', '.') }}</p>
                </div>
    ...
    <div class="mb-6" x-data="{ activeTab: '{{ request('type', 'all') }}' }">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-1 inline-flex gap-1">
            <a href="{{ route('admin.reports', array_merge(request()->except('type'), ['type' => 'all'])) }}" 
               @click="activeTab = 'all'"
            ...
            <a href="{{ route('admin.reports', array_merge(request()->except('type'), ['type' => 'kasir'])) }}" 
            ...
            <a href="{{ route('admin.reports', array_merge(request()->except('type'), ['type' => 'online'])) }}" 
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <form method="GET" class="flex gap-4 items-end">
            ...
            <div class="flex-1">
                ...
                <input type="date" name="date_from" value="{{ request('date_from') }}" max="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="flex-1">
                <label class="block text-sm font-medium mb-2">Sampai Tanggal</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" max="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="flex gap-2">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">Filter</button>
                <a href="{{ route('admin.reports', ['type' => request('type', 'all')]) }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300 transition">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm">
                        @if($transaction->shipping_method === 'kasir' && $transaction->member)
                            <div>
                                <div class="font-medium text-gray-900">{{ $transaction->member->name }}</div>
                                <div class="text-xs text-gray-500">{{ $transaction->member->email }}</div>
                            </div>
                    ...
                    <td class="px-6 py-4 text-sm font-semibold">Rp {{ number_format($transaction->total, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 text-sm capitalize">{{ $transaction->payment_method ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm">
                    ...
                    <td class="px-6 py-4 text-sm">
                        @if($transaction->is_verified)
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">
                                ✓ {{ $transaction->points_awarded ?? 0 }} poin
                            </span>
                        @else
                            <span class="text-gray-400">-</span>
                        @endif
