                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-3 mb-3">
                                    <div class="w-12 h-12 rounded-full flex items-center justify-center {{ $request->status === 'pending' ? 'bg-yellow-100' : ($request->status === 'approved' ? 'bg-green-100' : 'bg-red-100') }}">
                                    ...
                                    <div>
                                        <h3 class="font-bold text-lg text-gray-800">Order #{{ $request->order->order_number }}</h3>
                                    ...
                                    @endphp
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusColors[$request->status] }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-3">
                                        ...
                                        <p class="font-bold text-lg text-gray-800">Rp{{ number_format($request->order->total, 0, ',', '.') }}</p>
                                    </div>
                                    <div class="bg-gray-50 rounded-lg p-3">
                                        <p class="text-xs text-gray-500 mb-1">Status Order</p>
                                        <p class="font-semibold text-gray-800">{{ ucfirst($request->order->status) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded mb-4">
                                    ...
                                    <p class="text-blue-800">{{ $request->readable_reason }}</p>
                                    @if($request->reason_detail)
                                        <p class="text-sm text-blue-700 mt-2 italic">"{{ $request->reason_detail }}"</p>
                                    @endif
                                </div>
                                ...
                                @if($request->admin_note)
                                    <div class="bg-gray-50 border-l-4 border-gray-400 p-4 rounded mb-4">
                                        ...
                                        <p class="text-gray-700">{{ $request->admin_note }}</p>
                                        @if($request->reviewer)
                                            <p class="text-xs text-gray-500 mt-2">
                                                Direview oleh {{ $request->reviewer->name }} - {{ $request->reviewed_at->format('d M Y, H:i') }}
                                            </p>
                                        @endif
                                    </div>
                                @endif
                            ...
                            @if($request->status === 'pending')
                                <div class="flex flex-col gap-2">
                                    <button onclick="openReviewModal({{ $request->id }}, 'approve')" 
                                    ...
                                    <button onclick="openReviewModal({{ $request->id }}, 'reject')" 
                                    ...
                                    <a href="{{ route('admin.orders.show', $request->order->id) }}" 
