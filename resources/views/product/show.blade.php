...
<div class="max-w-7xl mx-auto py-6" x-data="productDetail()">
    ...
    <nav class="mb-6 text-sm text-gray-600">
        ...
        @else
            ...
            <span class="mx-2">/</span>
            <a href="{{ route('home', ['category' => $product->category->slug]) }}" class="hover:text-blue-600">{{ $product->category->name }}</a>
            <span class="mx-2">/</span>
            <span class="text-gray-900">{{ $product->title }}</span>
        @endif
    </nav>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-sm font-semibold text-blue-600">{{ $product->brand }}</span>
                </div>
                @endif
                <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $product->title }}</h1>
                @if($product->meta)
                    <p class="text-gray-600">{{ $product->meta }}</p>
                @endif
            </div>
            ...
            <div class="flex items-center gap-4 pb-4 border-b">
                <div class="flex items-center gap-1">
                    ...
                    @endfor
                    <span class="text-sm font-semibold ml-2">{{ $product->total_reviews > 0 ? $product->formatted_rating : 'Belum ada rating' }}</span>
                </div>
                @if($product->total_reviews > 0)
                    <span class="text-gray-400">|</span>
                    <a href="#reviews" class="text-sm text-blue-600 hover:underline">{{ $product->total_reviews }} Ulasan</a>
                @endif
                <span class="text-gray-400">|</span>
            ...
            <form method="POST" action="{{ route('cart.add', $product->id) }}" class="space-y-4">
                ...
                <div class="py-4 border-b">
                    ...
                    <div class="mb-4">
                        <label class="text-xs text-gray-600 mb-2 block">{{ $variant['type'] }}</label>
                        <div class="flex flex-wrap gap-2">
                            @foreach($variant['options'] as $optionIndex => $option)
                            <button type="button"
                                    @click="selectedVariants[{{ $variantIndex }}] = '{{ $option['value'] }}'"
                                    :class="selectedVariants[{{ $variantIndex }}] === '{{ $option['value'] }}' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-300 text-gray-700'"
                        ...
                        <input type="hidden" 
                               name="variant_{{ strtolower(str_replace(' ', '_', $variant['type'])) }}" 
                               x-model="selectedVariants[{{ $variantIndex }}]">
                    </div>
                    @endforeach
                </div>
                @endif
                ...
                @if(Auth::check() && Auth::user()->role === 'admin')
                    {{-- Admin tidak perlu quantity selector --}}
                @else
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm text-gray-600">Stok: <span class="font-semibold">{{ $product->stock }}</span></span>
                    </div>
                </div>
                @endif
            ...
            <div x-show="activeTab === 'specifications'">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex py-3 border-b">
                        ...
                        <span class="font-semibold text-gray-900">{{ $product->stock }} Buah</span>
                    </div>
                    <div class="flex py-3 border-b">
                    ...
                    <div class="flex py-3 border-b">
                        ...
                        <span class="font-semibold text-red-600">{{ $product->discount }}%</span>
                    </div>
                    @endif
                    <div class="flex py-3 border-b">
                    ...
                    <div class="flex py-3 border-b">
                        ...
                        <span class="font-semibold text-blue-600">{{ $product->formatted_final_price }}</span>
                    </div>
                    @endif
                    @if($product->variants && count($product->variants) > 0)
                    <div class="flex py-3 border-b col-span-2">
                        ...
                        <div class="flex-1">
                            @foreach($product->variants as $variant)
                                <div class="mb-2">
                                    <span class="font-semibold text-gray-900">{{ $variant['type'] }}:</span>
                                    <span class="text-gray-700">
                                        @foreach($variant['options'] as $option)
                                            {{ $option['value'] }}@if(!empty($option['price'])) (Rp {{ number_format($option['price'], 0, ',', '.') }})@endif{{ !$loop->last ? ', ' : '' }}
                                        @endforeach
                                    </span>
                                </div>
                            @endforeach
                    ...
                    @if($product->total_reviews > 0)
                        <div class="flex items-start gap-8">
                            <div class="text-center">
                                <div class="text-5xl font-bold text-gray-900 mb-2">{{ $product->formatted_rating }}</div>
                                ...
                                <div class="text-sm text-gray-600">{{ $product->total_reviews }} Ulasan</div>
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-1">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($review->updated_at && $review->created_at && $review->updated_at->diffInSeconds($review->created_at) > 60)
                                ...
                                <div x-show="!editingReview">
                                    ...
                                    @if($review->review)
                                        <p class="text-gray-700 text-sm mb-3">{{ $review->review }}</p>
                                    @endif
                                </div>
                                    ...
                                    <form method="POST" action="{{ route('reviews.update', $review->id) }}">
                                                ...
                                                @for($i = 1; $i <= 5; $i++)
                                                    <label class="cursor-pointer">
                                                        ...
                                                        <svg class="w-6 h-6 transition-colors"
                                                             :class="editRating >= {{ $i }} ? 'text-yellow-400' : 'text-gray-300'"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="review" rows="3" 
                                                      ...
                                                      maxlength="1000">{{ $review->review }}</textarea>
                                        </div>
                                ...
                                @if($review->admin_reply)
                                    <div class="mt-3 ml-4 pl-4 border-l-2 border-blue-500 bg-blue-50 p-3 rounded">
                                        <div class="flex items-center gap-2 mb-1">
                                            ...
                                            @if($review->replier)
                                                <span class="text-xs text-gray-500">({{ $review->replier->name }})</span>
                                            @endif
                                            <span class="text-xs text-gray-400">• {{ $review->replied_at->diffForHumans() }}</span>
                                        </div>
                                ...
                                @auth
                                    @if(auth()->user()->role === 'admin' && !$review->admin_reply)
                                        ...
                                        <div x-show="replyingReview" x-cloak class="mt-3 ml-4 pl-4 border-l-2 border-blue-500 bg-blue-50 p-3 rounded">
                                            <form method="POST" action="{{ route('reviews.reply', $review->id) }}">
                                                @csrf
...
function productDetail() {
    return {
        quantity: 1,
        @if($product->variants && count($product->variants) > 0)
        selectedVariants: {!! json_encode(array_map(function($variant) {
