...
<body class="bg-gray-50" x-data="{ sidebar: false, photoModal: false, photoUrl: '' }">
    ...
    <div x-show="sidebar" class="fixed inset-0 z-50 flex" 
        ...
        <div @mouseenter="sidebar=true" @mouseleave="sidebar=false" class="w-64 bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-200"
            ...
            <a href="{{ route('profile.show') }}" class="block p-6 hover:bg-gray-50 transition border-b border-gray-200">
            ...
            <a href="{{ route('admin.dashboard') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.dashboard') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                </svg>
                Dashboard
            </a>
                <a href="{{ route('admin.products') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.products*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                    </svg>
                    Produk
                </a>
                <a href="{{ route('admin.categories') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.categories*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                    </svg>
                    Kategori
                </a>
                <a href="{{ route('admin.reviews') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.reviews*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                    </svg>
                    Review Pelanggan
                </a>
                <a href="{{ route('admin.reports') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.reports*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    Laporan
                </a>
                <a href="{{ route('admin.orders') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.orders*') && !request()->routeIs('admin.orders.cancellations*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('admin.orders.cancellations') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.orders.cancellations*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="ml-auto bg-yellow-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $pendingCancellations }}</span>
                    @endif
                </a>
                <a href="{{ route('admin.accounts') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('admin.accounts*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                    </svg>
                    Akun
                </a>
                <a href="{{ route('admin.promotions') }}" class="py-2 px-3 rounded hover:bg-gray-100 text-gray-700 flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                    </svg>
                    Promosi
