...
<body class="bg-gray-50" x-data="{ sidebar: false, photoModal: false, photoUrl: '' }">
    ...
    <nav class="bg-white shadow-md">
        <div class="mx-auto px-4 py-3 flex items-center justify-between">
            <!-- Logo -->
            <a href="{{ route('kasir.dashboard') }}" class="flex items-center gap-2 hover:opacity-80 transition">
    ...
    <div x-show="sidebar" class="fixed inset-0 z-50 flex" 
        ...
        <div @mouseenter="sidebar=true" @mouseleave="sidebar=false" class="w-64 bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-200"
            ...
            <a href="{{ route('profile.show') }}" class="block p-6 hover:bg-gray-50 transition border-b border-gray-200">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-3">
                        ...
                        @endif
                        <div>
                            <div class="font-bold text-gray-800">{{ Auth::user()->name ?? 'Kasir' }}</div>
            ...
            <nav class="flex-1 flex flex-col gap-2 p-4 overflow-y-auto">
                <a href="{{ route('kasir.transaksi') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('kasir.transaksi*') || request()->routeIs('kasir.cetak-struk') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                    </svg>
                    Transaksi Penjualan
                </a>
                <a href="{{ route('kasir.riwayat') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('kasir.riwayat*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    Riwayat Transaksi
                </a>
                <a href="{{ route('kasir.pelanggan') }}" class="py-2 px-3 rounded hover:bg-gray-100 flex items-center gap-2 {{ request()->routeIs('kasir.pelanggan*') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
