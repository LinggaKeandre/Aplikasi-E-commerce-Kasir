...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="mb-6">
        <a href="<?php echo e(route('orders.index')); ?>" class="text-blue-600 hover:text-blue-800 flex items-center gap-2 mb-4">
    ...
    <?php if($order->cancellationRequest): ?>
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-orange-700 mt-1">
                        Status: 
                        <?php if($order->cancellationRequest->status === 'pending'): ?>
                            <span class="font-semibold">Menunggu Review Admin</span>
                        <?php elseif($order->cancellationRequest->status === 'approved'): ?>
                    ...
                    <p class="text-sm text-orange-600 mt-2">
                        <strong>Alasan:</strong> <?php echo e($order->cancellationRequest->readable_reason); ?>

                    </p>
                    <?php if($order->cancellationRequest->reason_detail): ?>
                        <p class="text-sm text-orange-600 mt-1">
                            <strong>Detail:</strong> <?php echo e($order->cancellationRequest->reason_detail); ?>

                        </p>
                    <?php endif; ?>
                    <?php if($order->cancellationRequest->admin_note): ?>
                        <div class="mt-3 bg-white border border-orange-200 rounded p-3">
                            <p class="text-sm font-semibold text-gray-800">Catatan Admin:</p>
                            <p class="text-sm text-gray-700 mt-1"><?php echo e($order->cancellationRequest->admin_note); ?></p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full <?php echo e(in_array($order->status, ['pending', 'processing', 'shipped', 'delivered']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center">
                    ...
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full <?php echo e(in_array($order->status, ['processing', 'shipped', 'delivered']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center">
                            <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                            </svg>
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600"><?php echo e(in_array($order->status, ['processing', 'shipped', 'delivered']) ? 'Pesanan sedang diproses' : 'Menunggu'); ?></p>
                        </div>
                    </div>
                    
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full <?php echo e(in_array($order->status, ['shipped', 'delivered']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center">
                            <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                            </svg>
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600"><?php echo e(in_array($order->status, ['shipped', 'delivered']) ? 'Paket dalam perjalanan' : 'Menunggu'); ?></p>
                        </div>
                    </div>
                    
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full <?php echo e($order->status === 'delivered' ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center">
                            <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                            </svg>
                        </div>
                        <div class="flex-1">
                            ...
                            <?php if($order->status === 'delivered' && $order->shipped_at): ?>
                                <p class="text-sm text-gray-600">Dikirim: <?php echo e($order->shipped_at->format('d M Y, H:i')); ?></p>
                            <?php else: ?>
                                <p class="text-sm text-gray-600">Estimasi: <?php echo e($order->estimated_delivery->format('d M Y, H:i')); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex-1">
                                <?php if($item->product): ?>
                                    <a href="<?php echo e(route('product.show', $item->product->slug)); ?>" class="group-hover:text-blue-600 transition-colors">
                                        <h3 class="font-semibold text-gray-800"><?php echo e($item->product_name); ?></h3>
                                    </a>
                                <?php else: ?>
                                    <h3 class="font-semibold text-gray-800"><?php echo e($item->product_name); ?></h3>
                                <?php endif; ?>
                                ...
                                <div class="mt-2 text-sm text-gray-600">
                                    <span>Rp<?php echo e(number_format($item->final_price, 0, ',', '.')); ?></span>
                                ...
                                <?php if($item->product_discount > 0): ?>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-xs line-through text-gray-400">Rp<?php echo e(number_format($item->product_price, 0, ',', '.')); ?></span>
                                        <span class="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded">-<?php echo e($item->product_discount); ?>%</span>
                                    </div>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="text-right">
                                ...
                                <?php if($order->status === 'delivered'): ?>
                                    <?php if($item->review): ?>
                                        ...
                                        <div class="mt-2">
                                            <div class="flex items-center gap-1 justify-end mb-1">
                                                <?php for($i = 1; $i <= 5; $i++): ?>
                                                    <svg class="w-4 h-4 <?php echo e($i <= $item->review->rating ? 'text-yellow-400' : 'text-gray-300'); ?>" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <button onclick="openReviewModal(<?php echo e($item->id); ?>, '<?php echo e($item->product_name); ?>')"
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <p class="text-gray-600 text-sm mt-1"><?php echo e($order->shipping_phone); ?></p>
                    <p class="text-gray-700 mt-3"><?php echo e($order->shipping_address); ?></p>
            ...
            <div class="sticky top-4 space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-gray-700">
                            <div>
                                ...
                                <?php if($order->free_shipping && $order->userVoucher): ?>
                                    <div class="text-xs text-green-600 font-medium mt-1">
                                        ✓ Voucher: <?php echo e($order->userVoucher->rewardVoucher->name ?? 'Gratis Ongkir'); ?>

                                    </div>
                                <?php endif; ?>
                            </div>
                            <span class="font-semibold" :class="{'text-green-600': <?php echo e($order->free_shipping ? 'true' : 'false'); ?> }">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg shadow-md p-6 border border-blue-200">
                    ...
                    <div class="bg-white rounded-lg p-3">
                        ...
                        <?php if($order->status === 'delivered' && $order->shipped_at): ?>
                            <p class="text-sm text-gray-600 mt-1">Dikirim pada: <?php echo e($order->shipped_at->format('d M Y, H:i')); ?></p>
                        <?php else: ?>
                            <p class="text-sm text-gray-600 mt-1">Estimasi tiba: <?php echo e($order->estimated_delivery->format('d M Y, H:i')); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
...
<div id="cancelModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4 sm:p-6">
        ...
        <form method="POST" action="<?php echo e(route('orders.request-cancellation', $order->id)); ?>" class="p-4 sm:p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
            <?php echo csrf_field(); ?>
            ...
            <div class="mb-4 sm:mb-6">
                ...
                <div class="space-y-2" x-data="{ selectedReason: '', showOtherInput: false }">
                    ...
                    <?php $__currentLoopData = $reasons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="flex items-start p-3 sm:p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition"
                               :class="selectedReason === '<?php echo e($key); ?>' ? 'border-red-500 bg-red-50' : 'border-gray-200'">
                            <input type="radio" 
                                   name="reason" 
            ...
            <div class="bg-gray-50 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
                ...
                <div class="text-xs sm:text-sm text-gray-700 space-y-1">
                    ...
                    <p><strong>Status:</strong> <?php echo e($statusLabels[$order->status]); ?></p>
                </div>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Aplikasi Kasir v2\resources\views/orders/show.blade.php ENDPATH**/ ?>