...
<div class="max-w-7xl mx-auto px-4 py-8" 
     x-data="{
        ...
        showAddressForm: <?php echo e(isset($user->address) && isset($user->city) && $user->address && $user->city ? 'false' : 'true'); ?>,
        useVoucher: true,
        selectedVoucher: null,
        availableVouchers: <?php echo e(Js::from($availableVouchers ?? [])); ?>,
        init() {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <?php if($user->address && $user->city): ?>
                        <div x-show="!showAddressForm">
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="font-semibold text-gray-800"><?php echo e($user->name); ?></p>
                                        <p class="text-gray-600 text-sm mt-1"><?php echo e($user->phone ?? '-'); ?></p>
                                        <p class="text-gray-700 mt-2"><?php echo e($user->address); ?></p>
                                        <p class="text-gray-600 text-sm"><?php echo e($user->city); ?><?php if($user->province): ?>, <?php echo e($user->province); ?><?php endif; ?> <?php if($user->postal_code): ?> <?php echo e($user->postal_code); ?><?php endif; ?></p>
                                    </div>
                                    <button type="button" @click="showAddressForm = true" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Ubah</button>
                                </div>
                            </div>
                            <input type="hidden" name="shipping_name" value="<?php echo e($user->name); ?>">
                            <input type="hidden" name="shipping_phone" value="<?php echo e($user->phone ?? ''); ?>">
                            <input type="hidden" name="shipping_address" value="<?php echo e($user->address); ?>">
                            <input type="hidden" name="shipping_city" value="<?php echo e($user->city); ?>">
                            <input type="hidden" name="shipping_province" value="<?php echo e($user->province ?? ''); ?>">
                            <input type="hidden" name="shipping_postal_code" value="<?php echo e($user->postal_code ?? ''); ?>">
                        </div>
                    <?php endif; ?>
                    ...
                    <div x-show="showAddressForm" x-cloak>
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="shipping_name" value="<?php echo e($user->name); ?>" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nomor Telepon *</label>
                                <input type="tel" name="shipping_phone" value="<?php echo e($user->phone ?? ''); ?>" required placeholder="08xxxxxxxxxx" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Lengkap *</label>
                                <textarea name="shipping_address" required rows="3" placeholder="Nama jalan, nomor rumah, RT/RW, kelurahan, kecamatan" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"><?php echo e($user->address ?? ''); ?></textarea>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Kota/Kabupaten *</label>
                                    <input type="text" name="shipping_city" value="<?php echo e($user->city ?? ''); ?>" required placeholder="Contoh: Jakarta" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Provinsi</label>
                                    <input type="text" name="shipping_province" value="<?php echo e($user->province ?? ''); ?>" placeholder="Contoh: DKI Jakarta" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kode Pos</label>
                                <input type="text" name="shipping_postal_code" value="<?php echo e($user->postal_code ?? ''); ?>" maxlength="10" placeholder="12345" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            </div>
                            <?php if($user->address && $user->city): ?>
                    ...
                    <div class="space-y-4">
                        <?php $__currentLoopData = $cart->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                                <img src="<?php echo e($item->product->image_url); ?>" class="w-20 h-20 object-cover rounded-lg border border-gray-200">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-800"><?php echo e($item->product->title); ?></h3>
                                    <p class="text-sm text-gray-600 mt-1">Rp<?php echo e(number_format($item->final_price, 0, ',', '.')); ?> × <?php echo e($item->quantity); ?></p>
                                    <?php if($item->variant_size || $item->variant_color): ?>
                                        <p class="text-xs text-gray-500 mt-1">
                                            <?php if($item->variant_size): ?>Size: <?php echo e($item->variant_size); ?><?php endif; ?>
                                            <?php if($item->variant_color): ?>, Warna: <?php echo e($item->variant_color); ?><?php endif; ?>
                                        </p>
                                    <?php endif; ?>
                                </div>
                                <div class="text-right">
                                    <p class="font-bold text-lg text-gray-800">Rp<?php echo e(number_format($item->total_price, 0, ',', '.')); ?></p>
                                </div>
                            </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6 sticky top-4">
                    ...
                    <div class="space-y-3 mb-4 pb-4 border-b border-gray-200">
                        <div class="flex justify-between text-gray-700">
                            <span>Subtotal</span>
                            <span class="font-semibold">Rp<?php echo e(number_format($subtotal, 0, ',', '.')); ?></span>
                        </div>
                        <div class="flex justify-between text-gray-700">
                            <span>Ongkir</span>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Aplikasi Kasir v2\resources\views/checkout/index.blade.php ENDPATH**/ ?>