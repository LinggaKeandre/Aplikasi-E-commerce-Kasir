<nav x-data="{showFilters:true}" class="bg-white shadow-sm sticky top-0 z-40">
    <div class="container mx-auto px-4 py-3.5 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="<?php echo e(route('home')); ?>" class="font-bold text-xl text-blue-600">Anashop</a>
        </div>

        <div class="flex-1 mx-4">
            <form action="<?php echo e(route('home')); ?>" method="get" class="w-full flex gap-2 items-center">
        ...
        <div class="flex items-center gap-3">
            <?php if(auth()->guard()->check()): ?>
                <a href="<?php echo e(route('cart.index')); ?>" class="p-2 hover:bg-gray-100 rounded-lg transition relative" title="Keranjang">
                    ...
                    <?php if($cartItemsCount > 0): ?>
                        <span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-semibold">
                            <?php echo e($cartItemsCount > 99 ? '99+' : $cartItemsCount); ?>
                ...
                <!-- Notifikasi Icon -->
                <a href="<?php echo e(route('notifications.index')); ?>" class="p-2 hover:bg-gray-100 rounded-lg transition relative" title="Notifikasi">
    ...
    <div x-data="{show:true,last:window.scrollY}" 
        ...
        <div class="container mx-auto px-4 flex gap-2 items-center">
            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('home', array_merge(request()->except('page'), ['category'=>$cat->slug]))); ?>"
                   class="px-4 py-1.5 rounded-lg text-sm font-medium transition-all <?php echo e(request('category')==$cat->slug ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-blue-50 hover:text-blue-600'); ?>">
                    <?php echo e($cat->name); ?>

                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php if(request('category')): ?>
                <a href="<?php echo e(route('home', request()->except(['category','page']))); ?>" class="ml-auto text-sm text-red-600 font-medium hover:text-red-700">Hapus</a>
            <?php endif; ?>
        </div>
    </div>
</nav>
...
<?php /**PATH C:\xampp\htdocs\Aplikasi Kasir v2\resources\views/components/navbar.blade.php ENDPATH**/ ?>