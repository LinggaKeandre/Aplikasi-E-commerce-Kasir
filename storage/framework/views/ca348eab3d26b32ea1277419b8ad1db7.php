...
<div class="py-6" x-data="pos()">
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a :href="'<?php echo e(route('kasir.customer-display')); ?>?session=' + sessionId" 
               target="_blank"
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="p-4 grid grid-cols-2 md:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto">
                    ...
                    ?>
                    <div x-show="searchProduct === '' || '<?php echo e(strtolower($product->title)); ?>'.includes(searchProduct.toLowerCase())"
                         @click="handleProductClick(<?php echo e($product->id); ?>, '<?php echo e($product->title); ?>', <?php echo e($finalPrice); ?>, <?php echo e($product->stock); ?>, <?php echo e($hasVariants ? 'true' : 'false'); ?>, <?php echo e($variantsJson); ?>, <?php echo e($product->discount ?? 0); ?>)" 
                         class="border border-gray-200 rounded-lg p-4 hover:border-green-500 hover:shadow-lg cursor-pointer transition">
                        <img src="<?php echo e($product->image_url); ?>" class="w-full h-32 object-cover rounded mb-2">
                        <h3 class="font-semibold text-sm text-gray-800 truncate"><?php echo e($product->title); ?></h3>
                        <?php if($hasVariants): ?>
                            <span class="inline-block bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded mb-1">Varian</span>
                        <?php endif; ?>
                        <?php if($hasVariants && $hasVariantPrice && $minPrice != $maxPrice): ?>
                            <p class="text-green-600 font-bold text-lg">Rp <?php echo e(number_format($minPrice, 0, ',', '.')); ?> ~ Rp <?php echo e(number_format($maxPrice, 0, ',', '.')); ?></p>
                        <?php elseif($hasVariants && $hasVariantPrice): ?>
                            <p class="text-green-600 font-bold text-lg">Rp <?php echo e(number_format($minPrice, 0, ',', '.')); ?></p>
                        <?php elseif($product->discount): ?>
                            <div class="flex items-center gap-2">
                                <p class="text-gray-400 line-through text-xs">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?></p>
                                <span class="bg-red-500 text-white text-xs px-1 rounded">-<?php echo e($product->discount); ?>%</span>
                            </div>
                            <p class="text-green-600 font-bold text-lg">Rp <?php echo e(number_format($finalPrice, 0, ',', '.')); ?></p>
                        <?php else: ?>
                            <p class="text-green-600 font-bold text-lg">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?></p>
                        <?php endif; ?>
...
function pos() {
    return {
            ...
            try {
                const payload = {
                    ...
                    cashier: {
                        ...
                        photo: '<?php echo e(auth()->user()->photo_url ?? ''); ?>'
                    }
                };
        ...
        async scanBarcode(barcode) {
            ...
            try {
                const response = await fetch('<?php echo e(route('kasir.scan-barcode')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        syncToCustomerDisplay() {
            ...
            localStorage.setItem('pos_cashier', JSON.stringify({
                name: '<?php echo e(Auth::user()->name); ?>',
                photo: '<?php echo e(Auth::user()->photo ? asset('storage/' . Auth::user()->photo) : ''); ?>'
            }));
        ...
        async processPayment() {
            ...
            fetch('<?php echo e(route("kasir.transaksi.process")); ?>', {
                method: 'POST',
                headers: {
        ...
        async searchMember() {
            ...
            try {
                const response = await fetch('<?php echo e(route('kasir.member.search')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        async verifyCode() {
            ...
            try {
                const response = await fetch('<?php echo e(route('kasir.member.verify')); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.kasir', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Aplikasi Kasir v2\resources\views/kasir/transaksi.blade.php ENDPATH**/ ?>